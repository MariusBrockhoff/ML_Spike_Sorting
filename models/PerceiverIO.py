...
class SelfAttention(tf.keras.Model):
    ...
    def call(self,inputs):
        ...
        state = self.attn(normed_inputs,normed_inputs,return_attention_scores=False) #state,vis / True

        state = self.attn_sum1([inputs,state])

        state_mlp = self.attn_mlp(state)

        return state_mlp    #return state_mlp, vis
...
class CrossAttention(tf.keras.Model):
    ...
    def call(self,inputs):
        ...
        state = self.x_attn(q_norm,k_norm,return_attention_scores=False) #state,vis / true

        #state = self.attn_sum1([q,state])
...
class AttentionBlock(tf.keras.layers.Layer):
  ...
  def call(self, latents, inputs):
    ...
    latents = self.CrossAttention([latents,inputs]) #latents, vis

    i = self.iter

    state_stack = self.state_stack

    state_0 = self.SelfAttention(latents) #[state_0,vis_0]

    state_stack = state_stack.write(0, state_0)

    cond = lambda i,ss: tf.less(i,self.depth)

    def body(i,state_stack): #body(i,state_stack,vis_stack):
      
      state = self.SelfAttention(state_stack.read(i-1)) #state,vis
...
class AutoPerceiver(tf.keras.Model):
  ...
  def call(self,inputs):
    ...
    output = self.Decoder(ENC_state) #logits
